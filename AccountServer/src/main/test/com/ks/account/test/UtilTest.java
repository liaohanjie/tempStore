...
public class UtilTest {
	...
	public void testJedisResultSetTOObject(){
		JedisResultSetTOObject(FightProp.class);
	}
	@Test
	public void testObjectToFieldMap(){
		objectToFieldMap(Notice.class);
	}
	
	@Test
	public void rowMapper(){
		rowMapper(Notice.class);
	}
	
	@Test
	public void testModelToVO(){
		modelToGwtVO(MonsterAwardVO.class);
	}
	...
	public void modelToGwtVO(Class<?> clazz){
		StringBuffer buff=new StringBuffer();
		buff.append("public "+clazz.getSimpleName()+"VO conv2"+clazz.getSimpleName()+"VO("+clazz.getSimpleName()+" model){" ).append("\r");
		buff.append("\r"+clazz.getSimpleName()+"VO vo=new "+clazz.getSimpleName()).append("VO();\r");
		Field[] fs = clazz.getDeclaredFields();				
		for(Field f : fs){
			if((f.getModifiers()&Modifier.STATIC)==0){
				String setM = "set"+f.getName().substring(0,1).toUpperCase()+f.getName().substring(1);
				String get = "get"+f.getName().substring(0,1).toUpperCase()+f.getName().substring(1);
				buff.append("	vo."+setM+"(model."+get+"());").append("\r");
			}
		}
		buff.append("return vo;\r");
		buff.append("}\r");
