...
public class CronSequenceGenerator {
	...
	private void parse(String expression) throws IllegalArgumentException {
		String[] fields = StringHelpers.tokenizeToStringArray(expression, " ");
		if (fields.length != 6) {
	...
	private String replaceOrdinals(String value, String commaSeparatedList) {
		String[] list = StringHelpers.commaDelimitedListToStringArray(commaSeparatedList);
		for (int i = 0; i < list.length; i++) {
			String item = list[i].toUpperCase();
			value = StringHelpers.replace(value.toUpperCase(), item, "" + i);
		}
		return value;
	}
	...
	private void setNumberHits(BitSet bits, String value, int min, int max) {
		String[] fields = StringHelpers.delimitedListToStringArray(value, ",");
		for (String field : fields) {
			if (!field.contains("/")) {
				...
				String[] split = StringHelpers.delimitedListToStringArray(field, "/");
				if (split.length > 2) {
