 ...
 * @date 2015年10月19日
 */
public class OppoHandler extends ChannelHandler {

	private static final Logger logger = LoggerFactory.get(OppoHandler.class);

	static final String AUTH_URL = PropertyUtils.SYS_CONFIG.get("sdk.oppo.auth.url");

	static final String APP_ID = PropertyUtils.SYS_CONFIG.get("sdk.oppo.app.id");

	static final String APP_KEY = PropertyUtils.SYS_CONFIG.get("sdk.oppo.app.key");

	static final String APP_SECRET = PropertyUtils.SYS_CONFIG.get("sdk.oppo.app.secret");
	...
	private String _requestAuth(String token, String accessTokenSecret) throws UnsupportedEncodingException {
		...
		String oauthSign = MacUtil.hamcSha1(method + "&" + URLEncoder.encode(AUTH_URL, UTF_8) + "&" + params, APP_SECRET + "&" + accessTokenSecret);
