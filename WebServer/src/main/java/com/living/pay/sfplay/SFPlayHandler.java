 ...
 * @date 2015年10月8日
 */
public class SFPlayHandler extends ChannelHandler {
	
	private static final Logger logger = LoggerFactory.get(SFPlayHandler.class);
	
	static final String AUTH_URL = "http://127.0.0.1";
	
	static final String AUTH_URL_DEBUG = "http://127.0.0.1";
	
	static final String APP_ID = "3951";
	
	static final String PRODUCT_ID = "1259";
	
	static final String APP_KEY = "2ao3JNL0";
	static final String PAY_KEY = "issht9GeUDdo";
	
	static final String UTF_8 = "UTF-8";
	
	private String _requestAuth(String umid, String token) {
		return HttpUtil.postRet(AUTH_URL, "appid=" + APP_ID + "&token=" + token + "&umid=" + umid + "&sig=" + MD5Util.decode(APP_ID + "|" + APP_KEY + "|" + token + "|" + umid), UTF_8, UTF_8);
	}
...
//	    // 加密串 sign=md5(order=xxxx&money=xxxx&mid=xxxx&time=xxxx&result=x&ext=xxx&key=xxxx) 
...
//	    return new StringPage("failure");
	...
	private boolean _validateNotify(String billno, Double amount, String mid, String time, Integer status, String orderId, String sign) {
		String checkSign = MD5Util.decode("order=" + billno + "&money=" + String.format("%.2f", amount) + "&mid=" + mid + "&time=" + time + "&result=" + status + "&ext=" + orderId + "&key=" + PAY_KEY);
